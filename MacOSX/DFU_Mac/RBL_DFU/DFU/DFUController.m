...
{
    NSLog(@"DFU Controller setPeripherl");
...
{
    NSString *extension = firmwareURL.pathExtension;
    const char* str = [extension UTF8String];
    if(memcmp(str, "hex", 3) == 0)
    {
        NSLog(@"Select a hex file.");
    ...
    else if(memcmp(str, "bin", 3) == 0)
    {
        NSLog(@"Select a bin file.");
...
- (void) setFirmwareBuffer:(NSData *)fwData
{
    self.firmwareData = fwData;
    self.appName = @"Arduino upload";
    self.appSize = self.firmwareData.length;
    
    NSLog(@"Arduino uploaded");
    NSLog(@"Set firmware with size %lu, notificationPacketInterval: %d", (unsigned long)self.firmwareData.length, self.notificationPacketInterval);
}
...
{
    NSLog(@"didReceiveResponse %d, forCommand %d, in state %d", response, opcode, self.state);
    switch (self.state)
    {
        case SEND_START_COMMAND:
            if (response == SUCCESS)
...
- (void) startTransfer
{
    ...
    if (self.state == IDLE)
    {
        ...
        {
            NSLog(@"Set notification packet interval manually!");
            self.notificationPacketInterval = 10;
        }
        
        NSLog(@"Notification packet interval: %d", self.notificationPacketInterval);
        
        NSLog(@"Wait 200 miliseconds before sending notification request...");
        usleep(200000);
